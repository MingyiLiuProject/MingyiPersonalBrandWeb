    ...
    <main>
      ...
      <section className="page-shell pb-16 pt-12 sm:pb-24 sm:pt-20">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
          <p className="micro-label flex items-center gap-2 text-[var(--muted)]">
            <span className="status-dot size-1.5 rounded-full bg-[var(--accent)]" aria-hidden="true" />
            Biomedical optics · Product systems
          </p>
          <p className="micro-label text-[var(--muted)]">Zurich · 47.3769° N</p>
        </div>
        <h1 className="display-serif max-w-5xl text-[clamp(3.4rem,7vw,6.8rem)] font-semibold leading-[0.92] tracking-[-0.065em]">
          Hey there, I&apos;m Mingyi <span className="text-[var(--accent)]" aria-hidden="true">✦</span>
        </h1>
        <div className="mt-9 grid gap-8 lg:grid-cols-[1.35fr_0.65fr] lg:items-end">
          <div className="max-w-3xl space-y-5 text-xl leading-8 tracking-[-0.025em] text-[var(--muted)] sm:text-2xl sm:leading-9">
            <p>I&apos;m a PhD researcher working with biomedical optics and laser systems.</p>
            <p>Before returning to research, I designed and built products at <span className="text-[var(--foreground)]">ByteDance</span> and <span className="text-[var(--foreground)]">Inceptio</span>.</p>
          </div>
          <p className="max-w-sm border-l border-[var(--line)] pl-5 text-sm leading-6 text-[var(--muted)]">
            This is my little garden for work, experiments, and ideas in progress.
          </p>
        </div>
      ...
      <section className="page-shell">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:auto-rows-[118px]">
          <Link href="/work" className="garden-card image-grain group min-h-[440px] bg-black lg:col-span-7 lg:row-span-4">
          ...
          <Link href="/work" className="garden-card image-grain group min-h-[360px] bg-black lg:col-span-5 lg:row-span-3">
          ...
          <article className="garden-card min-h-[360px] bg-[var(--butter)] p-7 lg:col-span-5 lg:row-span-3">
            ...
            <div className="mt-10 divide-y divide-black/15 border-t border-black/20">
              {buildItems.map((item) => (
                <Link key={item.index} href={item.href} className="group/block block py-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-black/50 transition-colors group-hover/block:text-black/70">{item.summary}</p>
                </Link>
              ))}
            </div>
          </article>

          <Link href="/work" className="garden-card image-grain group min-h-[360px] bg-black lg:col-span-7 lg:row-span-3">
          ...
          <Link href="/contact" className="garden-card group flex min-h-56 flex-col justify-between bg-[var(--paper)] p-6 lg:col-span-4 lg:row-span-2">
            <p className="micro-label text-[var(--accent)]">Say hello</p>
            <p className="display-serif text-3xl font-semibold">Have a question worth exploring? <span className="inline-block text-[var(--accent)] transition-transform group-hover:translate-x-1 group-hover:-translate-y-1">↗</span></p>
          </Link>
        </div>
      </section>
